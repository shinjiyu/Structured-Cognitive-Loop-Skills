...
ReAct 的**本意**是解决打断问题：每一步都有完整的思考-行动-观察记录，即使被打断也能恢复。

但它**无意中发现了**一件更重要的事：

> **不需要完整 Plan，短循环就能收敛到目标。**

每一轮推理只需要知道「现在在哪」和「要去哪」，就能决定下一步。Plan 的地位从「必需品」降级为「可选的辅助工具」。

**剩余局限**：ReAct 仍然发生在单个会话内，无法真正跨越时间。一旦会话结束，循环终止。
...
> Vibe Coding 让 AI 会"想"；ReAct 让 AI 会"循环思考"；Heartbeat 让 AI 会"活着"。
>
