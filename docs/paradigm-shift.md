...
Ralph Loop 的**本意**是解决打断问题：把 Agent 放进无限循环，即使某一轮失败或被中断，下一轮自然重启，状态通过 PROMPT.md / AGENT.md 等文件持久化。

但它**无意中打开了一个新方向**：

> **不需要完整 Plan，短循环就能收敛到目标。**

Ralph 的核心原则之一是：每轮只做一件事，**让 Agent 自己决定最重要的是什么**。这意味着你不需要在开始时就规划好一切——只需要提供目标和当前状态，Agent 每轮自主选择最优行动。

> "Frequent question: how do you plan? I don't. The models know what a compiler is better than I do. I just ask it." — Geoffrey Huntley

**剩余局限**：Ralph Loop 是纯粹的技术实现（bash + 文件），没有结构化的记忆层、心跳调度、或认知框架。它在 Greenfield 项目上极为有效，但在需要长期状态管理和跨领域决策的场景下仍需进化。
